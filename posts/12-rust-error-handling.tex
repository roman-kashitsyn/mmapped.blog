...
}{Sean Parent, \href{https://youtu.be/W2tWOdzgXHA?t=107}{\cite{C++ Seasoning}}}
...
}{The Rust Standard Library, \href{https://doc.rust-lang.org/std/macro.panic.html#when-to-use-panic-vs-result}{\cite{When to use \code{panic!} vs \code{Result}}}.}
...
pub enum SendMailError {
  \emph{/// One of the addresses passed to send\_mail is invalid.}
...
pub fn send\_mail(to: &str, cc: &[&str], body: &str) -> SendMailError { /* \ldots  */ }
\end{code}
\end{figure}

Note that our \code{send\_mail} function does at least two things: validating email addresses and sending emails.
...
\begin{enumerate}
  ...
  \item
    We replaced generic crypto errors with two specific cases: \code{MalformedPublicKey} and \code{SignatureVerificationFailed}.
