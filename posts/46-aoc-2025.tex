...
Rust is an excellent fit for Advent of Code:
...
it still requires over three years to solve a single instance.
...
This data structure maintains disjoint set partitions and allows merging them.
In this problem,
...
My computational geometry skills are weak,
...
and counts the number of polygon edges it crosses.
...
and exploring dynamic programming techniques,
but nothing worked.
...
\[ \left(2 k_1^\prime b_1 + \ldots + 2 k_n^\prime b_n = 2 t^\prime \right) \Rightarrow \left( k_1^\prime b_1 + \ldots + k_n^\prime b_n = t^\prime \right)\]
...
we precompute parity groups and consult them at every recursion level.
...
The problem seemed overwhelmingly complex,
and the input sizes were large,
...
If you want to dig deeper and solve the problem exactly,
Knuth's \href{https://en.wikipedia.org/wiki/Dancing_links}{dancing links algorithm} seems to be a promising direction to explore.
