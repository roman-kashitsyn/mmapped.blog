...
I liked the shorter form since keeping up with the puzzles for over three weeks in the previous years was challenging.
...
so I didn't need external dependencies\sidenote{sn-z3-dependency}{
  ...
  but then I found \href{#tenthmascot-solution}{a way} to do without it.
}.
...
In the first part part, $k = 2$ and the string length is $n = 100$,
so a brute-force $O(n^2)$ algorithm is fast enough.
The second part increses $k$ to $12$,
...
The shape of this combinatorial explosion is also a clue to a solution.
Binomial coefficients obey a recurrent formula that can be computed using \href{https://en.wikipedia.org/wiki/Dynamic_programming}{dynamic programming}.
...
we get the following recurrent formula:

\[M\left[k, i\right] = \operatorname{max}\left\{M\left[k, i-1\right], 10 \times M\left[k-1, i-1\right] + B\left[i-1\right]\right\}\]

Together with boundary conditions $M\left[k, 0\right] = 0$ and $M\left[0, i\right] = 0$, the formula translates to the following $O(n \times k)$ solution:
...
\href{https://adventofcode.com/2025/day/7}{This puzzle} asks us to simulate the physics of beam propagation.
The beam starts at the top and splits into two beams each time it reaches a \emph{splitter}.
The first part asks us to count how many splitters a beam can reach,
the second part---how many paths the beam can take through the maze.
...
which is still close to my heart.
Also, I like how the second part connects to the first part of \href{#day-11}{day 11 puzzle}.

My solution computes answers to both parts in one grid traversal.
It treats the grid as a \href{https://en.wikipedia.org/wiki/Directed_acyclic_graph}{direct acyclic graph}
...
The number of paths to a space is the sum of the numbers of paths to its parents.
...
I like this puzzle
because it's a perfect use case for one of my favorite data structures:
...
This data structure maintains disjoint set partitions
and allows merging them efficiently.
In our problem,
the set is the collection of points,
the partitions are connected components,
and the merge is adding an edge.
...
In the first part,
we must compute the sizes of three largest connected components
after adding \code{top\_k} edges.
We don't have to traverse the graph at any point
because the union-find data structure has all the information we need.
...
In the second part,
we must multiply the x-coordinates in the first edge that makes the graph connected.
Union-find makes it a child's play.
...
whose opposite corners coinside with the polygon corners.
In the second part,
...
checks that they satisfy the predicate passed as an argument,
and selects one with the largest area.
For the first part,
...
\begin{enumerate}
\item The polygon contains the rectangle's corners.
  Since two of them come from the polygon,
  we need to check only the complementary ones.
\item None of the rectangle's edges cross a polygon edge.
\end{enumerate}
...
It's equivalent to more wordy \code{y1.min(y2) <= y \&\& y < y1.max(y2)}.
The inequality is asymmetric (only one bound is inclusive) to ensure
that the edge is vertical (\code{y1 != y2})
and that we don't count intersections at polygon corners twice.
...
\href{https://adventofcode.com/2025/day/10}{This puzzle} is the most elaborate in the entire event.
...
My first instinct was to treat this puzzle as the shortest path problem in a state graph.
The vertices correspond to the current xor sum,
and edges correspond to vector additions ($\oplus b_i$).
The search starts at the zero vector and must reach vector $t$.
...
The shape of the second part is the same,
but now we're dealing with normal addition instead of xor,
and the target vector can contain arbitrary non-negative integers.
The objective is still to minimize the sum of $k_i$
that make the equation hold.

\[k_1 b_1 + \ldots + k_n b_n = t\]

I tried to adapt the graph-search solution
and explored dynamic programming techniques,
but my efforts yielded no results.
The values in the target vector were too large (over 50 on average in my input) for my programs to finish in a reasonable time.

So like \href{https://www.reddit.com/r/adventofcode/comments/1pivlyh/2025_day_10_every_time_a_problem_looks_remotely/}{many people} struggling with this problem,
I resorted to using \textsc{smt} solver \href{https://github.com/Z3Prover/z3}{z3}.
Rust has \href{https://crates.io/crates/z3}{nice bindings} to this library,
and my code worked correctly the first time it compiled.
...
The solution relies on the following observation:
If we know that all $k_i$ are even ($k_i = 2 k_i^\prime$),
then each component in the target vector must also be even.
So we can halve both $k_i$ and the target
and get a smaller instance of the same problem,
...
We can't be sure that all $k_i$ are even.
But we can make them even by subtracting at most one.
Which $k_i$ should we subtract one from?
We don't know, so we try all $2^n$ combinations and pick the outcome that yields the best result!
...
we can restrict our search to combinations that agree with the parity of the target components.
If an equation holds under normal arithmetic,
it must also hold under modulo-two arithmetic.
...
we have to evaluate only about $\frac{2^n}{2^m} = 2^{n-m}$ candidates at each step.
And indeed, all the instances in my input exhibit near perfect distribution\sidenote{sn-distribution}{
  I wrote a little program that generates random vectors and prints their parity group size distributions.
  Uniform coverage seems to be the norm.
}.
To take advantage of this reduction,
we can precompute parity groups and consult them at every level of recursion.
...
The idea of parity groups also leads to a simple algorithm for solving the first part:
Pick the smallest subset in the target parity group.
...
\href{https://adventofcode.com/2025/day/11}{This puzzle} asks us to count paths between two vertices in a \href{https://en.wikipedia.org/wiki/Directed_acyclic_graph}{direct acyclic graph}.
That's the same problem as the second part of \href{#day-07}{day seven},
but this time we deal with a general graph.
On day seven, the graph was embedded into a grid
and thus had a natural topological sorting.
This time, we have to compute a topological order ourselves.
...
In the second part,
we must compute the number of paths that start at vertex \code{svr},
end at \code{out}, and pass through vertices
\code{fft} and \code{dac} (in any order).
The most straightforward solution is to multiply path counts for segments of composite paths \code{svr -> fft -> dac -> out} and
\code{svr -> dac -> fft -> out}.
...
Of course, only one of expressions
\code{count\_paths(\&g, dac, fft)} and \code{count\_paths(\&g, fft, dac)}
can yield a non-zero value.

\section{day-12}{Day 12: Christmas tree farm}

\href{https://adventofcode.com/2025/day/12}{This puzzle} asks us
whether a rectangle of a given height and width can fit a given set of irregular tiles.
The problem is so overwhelmingly complex and the input sizes were so large
that I was sure there must be a trick.
After all, the last day is often cheesy.

One smoke test to check whether a rectangle can fit a set of tiles
is to check whether it's area is large enough.
If the rectangle's area is 360 units
and the set of tiles occupy 365 units,
the instance is unsolvable.

So I printed area ratios for all problem instances in my input.
They fell into two groups:
a group with the ratio of <80\%
and a group with the ratio of >100\%.
I assumed that if a rectangle \emph{could} fit the tiles,
it did.
Thank Santa, this guess gave the right answer.
