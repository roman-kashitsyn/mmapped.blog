...
}{Richard Feynman, \cite{What Do You Care What Other People Think?}}
...
}{Johan Granström, \cite{Tretise on Intuitionistic Type Theory, p. 2}}
...
The first map,
$\operatorname{encode}: \operatorname{Concept} \rightarrow \operatorname{Expression}$,
defines how the author picks words to communicate the concept.
The second map,
$\operatorname{decode}: \operatorname{Expression} \rightarrow \operatorname{Concept}$
...
$p(y | x)$ is the probability of receiving message $y$ if the sender sent message $x$.
...
and $I(C_r; C_s)$ is the mutual information computed for expression $e$.

\[\operatorname{best-name}\left(C_s\right) = \operatorname{argmax}_{e \in E}\left\{I\left(C_r; C_s\right)\right\}\]
...
}{John Baez, \href{https://math.ucr.edu/home/baez/act_course/lecture_5.html}{\cite{Applied Category Theory Course}}}
...
two partially ordered sets $A$ and $B$
and two monotonic maps $F$ and $G$ between these sets.
\href{https://en.wikipedia.org/wiki/Partially_ordered_set#Partial_orders}{Partial order}
means that we can compare some elements of each set using the $\leq$ operation that satisfies a few natural laws
...
A function $F$ is monotonic if it preserves relative order:
if $x \leq y$, then $F(x) \leq F(y)$.

Functions $F: A \rightarrow B$ and $G: B \rightarrow A$
form a Galois connection if they satisfy the following relation:
$F(a) \leq b \iff a \leq G(b)$ for all $a \in A$ and $b \in B$.
...
Our first set is the set of expressions $E$.
For expressions $e_1, e_2 \in E$
we say that $e_1 \leq e_2$
if $e_1$ is at least as expressive as $e_2$ \emph{given the reader context}.
If the expressions are equally expressive, we define the shorter element as smaller.
The reader context is a crucial component of the $\leq$ operator.
...
If we have concepts $c_1, c_2 \in C$
we say that $c_1 \leq c_2$ if $c_1$ is at least as specific as $c_2$.
For example, ``cat'' \leq  ``animal'':
...
$\operatorname{decode} : E \rightarrow C$
and the concept encoding function
$\operatorname{encode} : C \rightarrow E$.
...
$\operatorname{decode}(e) \leq c \iff e \leq \operatorname{encode}(c)$.
