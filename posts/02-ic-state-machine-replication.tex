...
As all blockchains, the Internet Computer relies on \href{https://en.wikipedia.org/wiki/State_machine_replication}{state machine replication} for fault-tolerance.
Unlike most blockchains,
it doesn't allow external parties to access or replicate its state.
This article explains the network's state machine design
and highlights design choices that make it fast and secure.

\section{state-machine}{The state machine}

Before we dive into the protocol internals,
let's define the \href{https://en.wikipedia.org/wiki/Finite-state_machine}{state machine}.
The nodes participating in the protocol are grouped into \emph{subnets}.
Each subnet runs a separate instance of the consensus protocol,
and but subnets can still exchange messages\sidenote{sn-xnet-protocol}{
  The \nameref{08-ic-xnet} article explains this exchange protocol in more detail.
}.
The purpose of consensus is to order inputs for the state machine.
...
\item
  ...
  their \href{https://webassembly.org/}{WebAssembly} modules,
...
\item
  \emph{Initial state} is trivial: no canisters, no messages, and no execution results.
\end{itemize}
...
results of user message execution,
...
\item
  \emph{Authentication:}
  A client can't trust any single node on the network,
...
is to represent it as a \href{https://en.wikipedia.org/wiki/Merkle_tree}{merkle tree},
which requires converting the data into a uniform format,
such as list or a tree.
...
that allows a client to authenticate the data it receives from any node.
...
Let's assume a user sends an ingress message with \textsc{id} \code{1355\ldots 48de}.
...
The only way for a dishonest node to manipulate the caller in this scheme
is to reply with an outdated state view,
so honest nodes always include into their output the \code{/time} path that
points to the timestamp of the block that triggered the tree construction.

\section{state-transfer}{State transfer}

\subsection{checkpoints}{Checkpoints}

If a node crashes, it loses its in-memory state and must recover
before it can participate in the protocol again.
To facilitate recovery,
nodes periodically write \emph{checkpoints}---serialized states---to disk.
If a node was offline for a few minutes,
it catches up by replaying blocks on top of the last checkpoint.
If it was offline for longer,
catching up by only replaying blocks would take too long,
so fetching the latest checkpoint from a peer
becomes a more efficient strategy.
The same strategy applies when a fresh node joins a subnet.

Most blockchains treat checkpoints as an implementation detail
and distribute them through a side channel.
In the Internet Computer, state sync is a part of the core protocol,
which streamlines node operations.
...
Most artifacts are under a few megabytes,
but the protocol can fetch arbitrary large blobs by slicing them into chunks.
State sync uses this machinery to disseminate checkpoints.

The peer-to-peer protocol combines \href{https://en.wikipedia.org/wiki/Push_technology}{push model} for metadata and \href{https://en.wikipedia.org/wiki/Pull_technology}{pull model} for data:
...
the summary of the checkpoint directory similar to the \code{info} section of a \href{https://en.wikipedia.org/wiki/Torrent_file}{\code{.torrent} file}.
A manifest consists of two tables:
the file table describes the file metadata,
the chunk table---the file contents.
...
\marginnote{mn-manifest-example}{
  ...
  the file table describes the file metadata,
  the chunk table---the contents of these files.
}
\begin{tabular}{rlrl}
  File & Path & Size & Hash \\
  \hrule
...
Replicas use manifests as blueprints for arranging the data on disk
and validating the chunks they receive.
A manifest root hash identifies the checkpoint artifact.
...
After every checkpoint, it creates an artifact called \emph{catch-up package}---the bill of materials a replica needs to participate in the protocol,
...
Let's imagine that a replica crashes after processing nine blocks
...
\item
  State Machine compares the manifest of local checkpoint 9 and the newly fetched manifest,
  classifying chunks from the latter as either \emph{locally available} or \emph{missing.}
\item 
  It copies all the locally available chunks into their proper positions.
\item
...
\item
  ...
  A catching up node has no other useful work to do,
...
This approach would make it easier to specify the protocol exactly
