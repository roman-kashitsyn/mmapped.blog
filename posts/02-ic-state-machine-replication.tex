...
so fetching the latest checkpoint from a peer is a more efficient strategy.
...
and comes back when the network moves on to state 100,
which happens to be the next scheduled checkpoint.
...
\item
  ...
  and its manifest hash is $H_{100}$.
\item
...
here is how the State Machine syncs from state 9 to 100:
...
\item
  ...
  A catching-up node is idle,
  so it should bear most of the costs.
\item
  It's sequential: 
