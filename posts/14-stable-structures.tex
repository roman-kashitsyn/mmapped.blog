...
}{Mike Acton, \href{https://youtu.be/rX0ItVEVjHc?t=1360}{\cite{Data-Oriented Design and C++}}, CppCon 2014}
...
}{Steve Maguire, \cite{Debugging the Development Process, p. 19}}
...
}{Niklaus Wirth, \cite{Algorithms + Data Structures = Programs, p. 1}}
...
\begin{code}[rust]
pub trait BoundedStorable: \href{#storable-trait}{\code{\b{Storable}}} {\label{bounded-storable-trait}
    /// The maximum slice length that \b{to_bytes} can return.
    ///
    /// \b{REQUIREMENT}: self.to_bytes().len() ≤ Self::MAX_SIZE as usize
    const MAX_SIZE: u32;\label{max-size-attribute}

    /// Whether all values of this type have the same length (equal to Self::MAX_SIZE)
    /// when serialized. If you are unsure about this flag, set it to \b{false}.
    ///
    /// \b{REQUIREMENT}: Self::IS_FIXED_SIZE ⇒ self.to_bytes().len() == Self::MAX_SIZE as usize
    const IS_FIXED_SIZE: bool;\label{is-fixed-size-attribute}
}
\end{code}
\end{figure}
...
}{Niklaus Wirth, \cite{Algorithms + Data Structures = Programs, p. xiii}}
...
\begin{code}[rust]
impl<T: \href{#storable-trait}{\code{Storable}}, M: \href{#memory}{\code{Memory}}> struct Cell<T, M> {
...
\begin{itemize}
  \item The \textsc{icrc-1} Ledger Archive canister persists its initialization arguments in a \href{https://github.com/dfinity/ic/blob/9cdb1e62bcd199f28ae0005ed3f762487a1454df/rs/rosetta-api/icrc1/archive/src/main.rs#L49}{cell}.
...
\begin{code}[rust]
impl<T: \href{#storable-types}{\code{BoundedStorable}}, Data: \href{#memory}{\code{Memory}}> struct Vec<T, Memory> {
...
}{Jay Kreps, \href{https://engineering.linkedin.com/distributed-systems/log-what-every-software-engineer-should-know-about-real-time-datas-unifying}{\cite{The Log}}}
...
\begin{itemize}
  \item
    The \textsc{icrc-1} Ledger Archive canister stores archived transactions in a \href{https://github.com/dfinity/ic/blob/9cdb1e62bcd199f28ae0005ed3f762487a1454df/rs/rosetta-api/icrc1/archive/src/main.rs#L58}{stable log}.
  \item
...
}{Niklaus Wirth, \cite{Algorithms + Data Structures = Programs, p. 250}}
...
}{Scott Meyers, \cite{Effective C++, third edition, Item 4, p. 26}}
...
thread_local! {
  static USERS: RefCell<\href{#stable-btree}{\code{StableBTreeMap}}<UserId, User, \href{#default-memory-impl}{\code{DefaultMemoryImpl}}>> =
...
impl<T> \href{#storable-trait}{\code{Storable}} for Cbor<T>
where T: serde::Serialize + serde::de::DeserializeOwned
{
...
thread_local! {
  static METADATA: RefCell<\href{#stable-cell}{\code{StableCell}}<Cbor<Option<Metadata>>, RM>> =
  ...
  static BALANCES: RefCell<\href{#stable-btree}{\code{StableBTreeMap}}<Account, Amount, VM>> =
    MEMORY_MANAGER.with(|mm| {
      RefCell::new(StableBTreeMap::init(mm.borrow().get(\b{BALANCES_MEM_ID})))
    });

  static TX_LOG: RefCell<\href{#stable-log}{\code{StableLog}}<Cbor<Tx>, VM, VM>> =
    MEMORY_MANAGER.with(|mm| {
