...
}{\bibref[Chapter 14]{ousterhout-philosophy}}
...
func main() {
  ...
  // Note that all these operations give us MetricApples back,
...
\epigraph{A constraint on component design leads to freedom and power when putting those components together into systems.}{\bibref{bjarnason-constraints}}
...
trait VectorSpace {
  /// The scalar type is usually the same as the \href{#representation-type}{Representation} type.
  type Scalar;
...
For example, acceleration would have label $\mathrm{distance} \times  \mathrm{time}^{-2}$, and the \textsc{usd}/\textsc{eur} \href{https://en.wikipedia.org/wiki/Currency_pair}{pair} exchange rate would have label $\mathrm{eur} \times  \mathrm{usd}^{-1}$.
