...
Newtypes wrap an existing type and allow the programmer to inherit some of the operations from the underlying type.
...
Typedefs introduce a new name for an existing type, inheriting all of the underlying type operations.
...
func main() {
  ...
  // Apples times apples gives apples squared.
  // Dividing apples gives a dimensionless number.
...
Newtypes and typedefs are versatile and very useful in practice, but they approach the problem in a way that's too simplistic and mechanical.
...
\begin{figure}
\marginnote{mn-identity-types}{
  The minimal interface for identity types.
}
\begin{code}
trait Eq {
...
\href{#typdefs}{Typedefs}, on the other hand, impose too much structure, allowing the programmer to accidentally add and subtract numeric identities.
But given the choice, typedefs are still a safer option than using raw integers or strings.
...
There is nothing wrong with a banking app increasing a dollar amount by ten percent or a disk utility dividing the total number of bytes occupied by the file count.
...
\begin{figure}
\marginnote{mn-amount-types}{
  The minimal interface for amount types.
}
\begin{code}
trait Ord: Eq {
...
Positions refer to a point in space, such as a timestamp or a geographical coordinate.
...
\begin{figure}
\marginnote{mn-locus-types}{
  The minimal interface for locus types.
}
\begin{code}
...
A physics simulation might need to multiply a time interval by a velocity to get a distance.
...
We discussed four common classes of domain types that fit naturally in almost any application: \href{#identity-types}{identity types}, \href{#amount-types}{amount types}, \href{#locus-types}{locus types}, and \href{#quantities}{quantities}.
Each type classes is a little gem, a universal structure akin to a \href{https://en.wikipedia.org/wiki/Software_design_pattern}{design pattern}.
