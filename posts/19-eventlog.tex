...
\begin{itemize}
    ...
    \item
    Fees owed to the \href{https://thepaypers.com/expert-opinion/know-your-transaction-kyt-the-key-to-combating-transaction-laundering--1246231}{Know Your Transaction} (\textsc{kyt}) service providers.
\end{itemize}
...
\begin{figure}[grayscale-diagram]
  \marginnote{mn-traditional-upgrade}{
    ...
    The canister applies \emph{state transitions} $T_i$ to its states $S_i$ on the Wasm heap (designated with a circle) and marshals the state through stable memory on upgrades.
...
\begin{figure}[grayscale-diagram]
  \marginnote{mn-stable-memory}{
    ...
    The canister applies \emph{state transitions} $T_i$ to its states $S_i$ persisted in stable memory.
...
\begin{figure}[grayscale-diagram]
  \marginnote{mn-event-log-upgrade}{
    ...
    As in the traditional scheme, the canister applies state transitions $T_i$ to its states on the Wasm heap, but it also records the state transitions to an append-only log data structure in stable memory.
