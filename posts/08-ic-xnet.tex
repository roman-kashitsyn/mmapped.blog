...
\begin{itemize}
  ...
  \item
    The signals for the \emph{reverse} stream (\math{Y → X}): for each message index in the reverse stream, \math{Y} tells whether \math{X} can garbage collect the message (an \sc{ack} signal) or should reroute the message (a \sc{reject} signal).
    A \sc{reject} signal indicates that the destination canister moved, so \math{X} should route the message into another stream.
\end{itemize}
