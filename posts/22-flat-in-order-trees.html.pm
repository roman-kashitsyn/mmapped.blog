...
◊(define-meta title "Flat in-order binary trees")
...
◊section{
◊section-title["sec-background"]{Background}
◊p{
  Authenticated data structures are essential in distributed systems (e.g., version control systems and blockchains).
  Binary ◊a[#:href "https://en.wikipedia.org/wiki/Merkle_tree"]{Merkle trees} are among the most common approaches to authenticated data structures.
}
◊p{
  Building a Merkle tree from scratch for an arbitrary data structure is straightforward and can be mechanized◊sidenote["sn-auth-ds-generically"]{
    See, for example, ◊a[#:href "https://www.cs.umd.edu/~mwh/papers/gpads.pdf"]{Authenticated Data Structures, Generically} by Andrew Miller et al.
  }.
...
◊figure[#:class "grayscale-diagram"]{
  ...
  ◊(embed-svg "images/22-new-item.svg")
}
...
◊p{
  ...
  Finding an ◊a[#:href "https://en.wikipedia.org/wiki/Implicit_data_structure"]{implicit encoding} mapping the tree onto an array of ◊math{2N-1} hashes reduces storage space significantly.
}

◊p{
  The most common approach to packing a tree into an array used in binary heaps and ◊a[#:href "https://cp-algorithms.com/data_structures/segment_tree.html"]{segment trees} is to place children of node at index ◊math{i} at positions ◊math{2i+1} and ◊math{2i+2}.
...
◊figure[#:class "grayscale-diagram medium-size"]{
  ◊marginnote["mn-binary-heap"]{
    Representing a binary tree as an array using the ◊a[#:href "https://en.wikipedia.org/wiki/Binary_heap#Derivation_of_index_equations"]{binary heap array indexing}.
    If a tree node maps to index ◊math{i}, then its left and right children map to indices ◊math{2i+1} and ◊math{2i+2} respectively.
  }
  ◊(embed-svg "images/22-binary-heap.svg")
}

◊p{
  The ◊a[#:href "https://www.datprotocol.com/"]{DAT protocol} pioneered an alternative approach, ◊a[#:href "https://dat-ecosystem-archive.github.io/book/ch01-01-flat-tree.html"]{flat in-order trees}.
...
◊figure[#:class "grayscale-diagram medium-size"]{
  ◊marginnote["mn-in-order-tree"]{
    Representing a binary tree as an array using the flat in-order indexing.
  }
  ◊(embed-svg "images/22-in-order-tree.svg")
}
...
◊section{
◊section-title["sec-nomenclature"]{Nomenclature}
◊p{
  We will consider two types of trees.
  First, we'll analyze the case of ◊em{perfect binary trees} (◊code{PBT}), i.e., binary trees in which all interior nodes have two children and all leaves have the same depth.
...
◊p{
  ...
  There is no established name for such trees, so we'll call them ◊em{left-perfect binary trees} (◊code{LPBT}).
...
◊figure[#:class "grayscale-diagram medium-size"]{
  ...
  ◊(embed-svg "images/22-left-perfect-tree.svg")
}
}

◊section{
◊section-title["sec-addressing"]{Addressing tree nodes}
◊p{
...
◊p{
  ◊smallcaps{theorem}: in a flat in-order perfect binary tree of depth ◊math{D}, node's index ◊math{i} has the following structure:
}
◊ul[#:class "arrows"]{
  ◊li{
...
◊figure[#:class "grayscale-diagram medium-size"]{
  ...
  ◊(embed-svg "images/22-addressing-scheme.svg")
}

◊p{
  ◊smallcaps{proof}: by induction on the tree depth ◊math{D}.
}
...
◊figure[#:class "grayscale-diagram"]{
  ...
  ◊(embed-svg "images/22-depth-two-example.svg")
}
...
◊figure[#:class "grayscale-diagram"]{
  ...
  ◊(embed-svg "images/22-visual-proof.svg")
}

◊p{
  The root of a ◊math{D+1}-tree resides at index ◊math{2◊sup{D}-1}, so its binary representation is ◊math{011..1}, with precisely ◊math{D} least-significant bits set.
...
◊section{
◊section-title["sec-fbt"]{Traversing perfect binary trees}
...
◊p{
  We'll need two primitive bit-twiddling operations for the tree navigation: locating the last zero bit of a number and rounding a number up to a power of two◊sidenote["sn-examples-c"]{
    All code in this article is in the ◊a[#:href "https://en.wikipedia.org/wiki/C_(programming_language)"]{C programming language} and complies with the ◊a[#:href "https://en.wikipedia.org/wiki/C11_(C_standard_revision)"]{C11 standard}.
  }.
}
...
uint64_t RoundUpPowerOf2(uint64_t n) {
    // See ◊a[#:href "https://graphics.stanford.edu/~seander/bithacks.html#RoundUpPowerOf2"]{Bit Twiddling Hacks}.
    n--;
    n |= n >> 1;
    n |= n >> 2;
    n |= n >> 4;
...
◊figure[#:class "grayscale-diagram medium-size"]{
  ...
  ◊(embed-svg "images/22-fbt-parent.svg")
}
...
◊section{
◊section-title["sec-lpbt"]{Traversing left-perfect binary trees}
...
◊figure[#:class "grayscale-diagram"]{
  ...
  ◊(embed-svg "images/22-cbt-structure-change.svg")
}
...
◊figure[#:class "grayscale-diagram"]{
  ◊marginnote["mn-missing-nodes"]{
    ...
    Following node's ◊quoted{virtual} parents until we hit a ◊quoted{real} node gives us the node's true parent.
  }
  ◊(embed-svg "images/22-missing-nodes.svg")
}
...
◊p{
  Alternatively, if ◊code{PBT_RightChild}'s result falls outside of the tree, we can follow the left links in the ◊quoted{virtual} perfect tree until we hit the tree bounds again.
}
...
◊section{
◊section-title["sec-extensible-segment-trees"]{Extensible segment trees}

◊p{
  Merkle trees are a special case of ◊a[#:href "https://cp-algorithms.com/data_structures/segment_tree.html"]{segment trees}.
...
◊p{
  ...
  The same approach will work for any problem where node elements and the aggregation operation form a ◊a[#:href "https://en.wikipedia.org/wiki/Semigroup"]{semigroup}.
}

◊p{
  We'll need a new operation to support efficient range queries: finding the ◊a[#:href "https://en.wikipedia.org/wiki/Lowest_common_ancestor"]{lowest common ancestor} (LCA) of two leaves in a left-perfect binary tree.
  The ◊a[#:href "#sec-addressing"]{tree addressing scheme} suggests a simple way to compute LCA for leaves because their paths from the root node are aligned.
...
◊figure[#:class "grayscale-diagram"]{
  ...
  ◊(embed-svg "images/22-lca-sum.svg")
}
