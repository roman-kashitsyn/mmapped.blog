...
}{John Hughes, \href{https://www.sciencedirect.com/science/article/pii/S0167642399000234}{\cite{Generalising monads to arrows}}}
...
let make_table t =\label{ref-make-table}
    let open Box in
...
(** Creates an h\times w box filled with character c. *)
let fill c h w = Array.make h (String.make w c)\label{ref-fill}

(** Creates a 1\times 1 box containing character c. *)
let of_char c = fill c 1 1\label{ref-of-char}

(** Creates an h\times w box filled with spaces. *)
...
let rec beside ?(align:vertical = `Center) l r =\label{ref-beside}
...
and above ?(align:horizontal = `Center) t b =\label{ref-above}
...
and widen ?(align:horizontal = `Center) w b =\label{ref-widen}
    if width b >= w then b
...
and heighten ?(align:vertical = `Center) b h =\label{ref-heighten}
...
and hconcat ?(align:vertical = `Center) boxes =\label{ref-hconcat}
    Array.fold_left (beside ~align) empty boxes

(** Stacks an array of boxes vertically. *)
and vconcat ?(align:horizontal = `Center) boxes =\label{ref-vconcat}
    Array.fold_left (above ~align) empty boxes

(** Arranges a 2-D array of boxes. *)
let grid g = Array.map hconcat g |> vconcat\label{ref-grid}
