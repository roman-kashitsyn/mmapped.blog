...
let empty =
  { input = None; output = None; root = "https://mmapped.blog"; serve_port = None }

let rec parse_args acc = function
  | [] -> Ok acc
  | "-input" :: dir :: rest ->
    parse_args { acc with input = Some dir } rest
  | "-output" :: dir :: rest ->
    parse_args { acc with output = Some dir } rest
  | "-root" :: url :: rest ->
    parse_args { acc with root = url } rest
  | "-serve" :: port_str :: rest ->
    (match int_of_string_opt port_str with
     | Some p -> parse_args { acc with serve_port = Some p } rest
     | None -> Error ("Invalid port number: " ^ port_str))
  | "-f" :: _ :: _ ->
    Error "Single-file rendering not yet implemented"
  | unknown :: _ ->
...
let run (p : parsed) : unit =
  ...
  | Some port ->
    let config = { site_input = input; site_output = ""; site_root = p.root } in
  ...
  | None ->
    ...
    | None ->
      ...
      exit 1
    | Some out ->
      let config = { site_input = input; site_output = out; site_root = p.root } in
