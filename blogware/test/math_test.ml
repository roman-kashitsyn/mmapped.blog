...
let parse_math input : (math_node list, Error.parse_error) result =
  ...
  | Ok _ -> Error { Error.pe_pos = Parser_pos.make "" 0 0
...
let show_math_list ns =
  let rec go = function
    ...
    | Math_cmd (n, args) -> "cmd(" ^ n ^ "," ^ String.concat "," (List.map go args) ^ ")"
...
let tests : Test_framework.t list =
  group "math"
    ...
    ; math_expect "minus sign"
        "-1"
        "op(-) num(1)"
    ; math_expect "subtraction is binary"
        "a-1"
        "text(a) op(-) num(1)"
    ; math_expect "subscript"
        "x_i"
        "term(text(x),text(i),_)"
    ; math_expect "superscript"
        "x^2"
        "term(text(x),_,num(2))"
    ; math_expect "frac command"
        "\\frac{a}{b}"
