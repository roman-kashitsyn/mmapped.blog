...
let elab_ok name input check_blocks : Test_framework.t =
  test name (fun () ->
    match Parser.parse_document ~source_name:"<test>" input with
...
let tests : Test_framework.t list =
  group "elaborate"
    ...
    ; elab_ok "section wrap"
        ...
        (fun blocks ->
          match blocks with
          | [Section (Some ("id", [Str "Title"]), [Para [Str "body"]])] -> Pass
          | _ -> Fail "expected named section with body")

    ; elab_ok "anonymous preamble before section"
        "\\begin{document}pre\\section{id}{T}body\\end{document}"
        (fun blocks ->
          match blocks with
          | [ Para [Str "pre"]
    ...
    ; elab_ok "strong inline"
        "\\begin{document}\\b{bold}\\end{document}"
        (fun blocks ->
          match blocks with
    ...
    ; elab_ok "hrule block"
        "\\begin{document}\\hrule\\end{document}"
        (fun blocks ->
          match blocks with
          | [HRule] -> Pass
          | _ -> Fail "expected hrule")

    ; elab_ok "itemize list"
        "\\begin{document}\\begin{itemize}\\item a\\item b\\end{itemize}\\end{document}"
        (fun blocks ->
          match blocks with
          | [Bullet_list (Arrows, items)]
            when List.length items = 2 -> Pass
