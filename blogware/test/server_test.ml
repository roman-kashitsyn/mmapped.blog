...
let tests : Test_framework.t list =
  group "server"
    [ test "parse_request_line GET /" (fun () ->
        let (m, p) = Server.parse_request_line "GET / HTTP/1.1\r\nHost: x\r\n\r\n" in
    ...
    ; test "content_type_for png" (fun () ->
        assert_equal_string "image/png" (Server.content_type_for "/images/x.png"))
    ...
    ; test "build_response has status line" (fun () ->
        ...
        if String.length r >= 15
           && String.sub r 0 15 = "HTTP/1.1 200 OK" then Pass
        else Fail r)
