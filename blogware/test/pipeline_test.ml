...
let parse src =
  Parser.parse_document ~source_name:"<test>" src
...
let assert_parse_error_msg src expected : check =
  ...
  | Error err ->
    assert_equal_string expected (Error.format_parse_error src err)

let assert_elab_error_msg src expected : check =
  match elaborate_src src with
  | Ok _ -> Fail "expected elab error, got Ok"
  | Error err ->
    assert_equal_string expected (Error.format_elab_error src err)
