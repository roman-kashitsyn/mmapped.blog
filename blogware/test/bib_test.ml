...
let parse input = Bib.parse ~source_name:"<test>" input
...
let norm_entry = function
  | Book b ->
      Book
        {
          book_key = t b.book_key;
          book_author = t b.book_author;
          book_title = t b.book_title;
          book_year = ot b.book_year;
          book_url = ot b.book_url;
          book_isbn = ot b.book_isbn;
        }
  | Article a ->
      Article
        {
          article_key = t a.article_key;
          article_author = t a.article_author;
          article_title = t a.article_title;
          article_journal = ot a.article_journal;
          article_year = ot a.article_year;
          article_url = ot a.article_url;
        }
  | Phdthesis p ->
      Phdthesis
        {
          phdthesis_key = t p.phdthesis_key;
  ...
  | Blog b ->
      Blog
        {
          blog_key = t b.blog_key;
          blog_title = t b.blog_title;
          blog_author = ot b.blog_author;
          blog_url = t b.blog_url;
        }
  | Talk k ->
      Talk
        {
          talk_key = t k.talk_key;
          talk_author = t k.talk_author;
          talk_title = t k.talk_title;
          talk_year = ot k.talk_year;
          talk_url = ot k.talk_url;
        }
  | Podcast p ->
      Podcast
        {
          podcast_key = t p.podcast_key;
          podcast_title = t p.podcast_title;
          podcast_author = ot p.podcast_author;
          podcast_url = t p.podcast_url;
        }
  | Misc m ->
      Misc
        {
          misc_key = t m.misc_key;
          misc_title = t m.misc_title;
          misc_author = ot m.misc_author;
          misc_year = ot m.misc_year;
          ...
          Book
            {
              ...
              book_url =
                Some (s "https://www-cs-faculty.stanford.edu/~knuth/taocp.html");
              book_isbn = None;
            };
        ];
          ...
          Phdthesis
            {
              ...
              phdthesis_title =
                s "The Purely Functional Software Deployment Model";
              phdthesis_school = Some (s "Utrecht University");
              phdthesis_year = Some (s "2006");
              phdthesis_url =
                Some (s "https://edolstra.github.io/pubs/phd-thesis.pdf");
            };
        ];
      parse_ok "misc"
        {|
          ...
          Misc
            {
              ...
              misc_title =
                s
                  "My Observations During the Explosion at Trinity on July 16, \
                   1945";
...
|}
      ...
      parse_ok "nested braces in value" {|@misc{x, title = {A {Nested} Title}}|}
        [
          Misc
            {
              misc_key = s "x";
...
|}
      ...
      parse_ok "empty file" "" [];
      parse_ok "key with dots and colons" {|@misc{rfc:2119.v2, title = {RFC}}|}
        [
          Misc
            {
              misc_key = s "rfc:2119.v2";
              misc_title = s "RFC";
              misc_author = None;
              misc_year = None;
              misc_url = None;
      ...
      parse_fails "unterminated value" {|@misc{x, title = {unclosed|};
      parse_fails "missing closing brace" {|@misc{x, title = {ok}|};
      parse_fails "unknown entry type" {|@thesis{x, title = {Foo}}|};
      parse_fails "book missing author" {|@book{x, title = {Foo}}|};
      test "missing field error mentions entry type" (fun () ->
          match parse {|@book{my-key, title = {Foo}}|} with
          | Ok _ -> Fail "expected parse error"
          | Error e ->
              if Strings.is_infix_of "@book{my-key}" e.pe_message then Pass
              else Fail ("unexpected error message: " ^ e.pe_message));
      parse_fails "blog missing url" {|@blog{x, title = {Foo}}|};
