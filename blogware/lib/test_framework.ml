...
let test (name : string) (action : unit -> check) : t = { name; run = action }

(* Prefix a list of tests with [group_name ^ "/"]. *)
let group (group_name : string) (tests : t list) : t list =
  List.map (fun tc -> { tc with name = group_name ^ "/" ^ tc.name }) tests
...
let assert_equal_string (expected : string) (actual : string) : check =
  if expected = actual then Pass
  else Fail (Printf.sprintf "Expected:\n  %S\nGot:\n  %S" expected actual)
...
let assert_ok (r : ('a, 'e) Stdlib.result) (k : 'a -> check) : check =
  match r with Ok x -> k x | Error _ -> Fail "expected Ok, got Error"

let assert_error (r : ('a, 'e) Stdlib.result) (k : 'e -> check) : check =
  match r with Error e -> k e | Ok _ -> Fail "expected Error, got Ok"
...
let run_tests ?(verbose = false) (tests : t list) : int =
  ...
  List.iter
    (fun tc ->
      ...
      match r with
      | Pass -> incr passed
      | Fail msg -> failures := (tc.name, msg) :: !failures)
    tests;
