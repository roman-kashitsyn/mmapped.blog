...
let rec render_math_node (n : math_node) : Html.t =
  ...
  | Math_op (op, stretchy) ->
    if stretchy
    then mo_ [] (raw (escape_math_text op))
  ...
  | Math_frac (num, denom) ->
    mfrac_ []
      (mrow_ [] (render_math_node num)
       ++ mrow_ [] (render_math_node denom))
  | Math_term (nucleus, msub, msup) ->
    render_math_term nucleus msub msup
...
and render_math_cmd name args =
  ...
  | "binom", [n; k] ->
    mrow_ []
      (mo_ [] (raw "(")
       ++ mfrac_ [attr "linethickness" "0"]
            (mrow_ [] (render_math_node n)
             ++ mrow_ [] (render_math_node k))
       ++ mo_ [] (raw ")"))
  | "operatorname", [Math_op (s, stretchy)] ->
    if stretchy
    then mo_ [] (raw (escape_math_text s))
  ...
  | _, [] ->
    ...
    mo_ [] (raw repl)
  | _, _ ->
    mo_ [] (raw name) ++ concat (List.map render_math_node args)
...
let rec render_mathml_node (n : node) : Html.t =
  ...
  | NCmd (_, "mtable", _,
          Arg_align (_, spec) :: Arg_nodes (_, ns) :: _) ->
