...
let elab_error (pos : Parser_pos.t) (msg : string) : 'a result_ =
...
let try_replacement s =
  let rec go = function
    | [] -> None
    | (pref, rep) :: rest ->
      if string_starts_with s pref
      then Some (rep, String.length pref)
      else go rest
  in
...
let parse_day pos t : Date.t result_ =
  ...
  | Some d -> Ok d
  | None ->
    elab_error pos ("invalid date: " ^ t ^ " (expected YYYY-MM-DD)")

(* Flatten text-like nodes back to a string for simple arg contexts. *)
let rec node_text_of_nodes ns =
  String.concat "" (List.map node_text ns)
...
type flat_block =
  ...
  | AnonSectionMark
  | SubsectionMark of Parser_pos.t * string * inline list
...
let metadata_cmd_set =
  sset_of_list
    [ "documentclass"; "title"; "subtitle"; "date"; "modified"
    ; "keyword"; "reddit"; "hackernews"; "lobsters" ]

let mathml_subcmd_set =
  sset_of_list
    [ "mi"; "mn"; "mo"; "mo*"; "msup"; "msub"; "mtext"; "mrow"
    ; "mtable"; "mtr"; "mtd"; "munderover"; "msubsup" ]
...
let rec classify_node (n : node) : node_class result_ =
  ...
  | NCmd (pos, "subsection", _, Arg_symbol (_, anchor) :: Arg_nodes (_, title) :: _) ->
    let* ils = elaborate_inlines title in
    Ok (CSection (SubsectionMark (pos, anchor, ils)))

  | NCmd (_, "b", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Strong ils))
  | NCmd (_, "emph", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Emph ils))
  | NCmd (_, "u", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Underline ils))
  | NCmd (_, "textsc", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Small_caps ils))
  | NCmd (_, "strikethrough", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Strikethrough ils))
  | NCmd (_, "code", opts, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_code_inlines ns in Ok (CInline (Code (opts, ils)))
  | NCmd (_, "kbd", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Kbd ils))
  | NCmd (_, "sub", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Sub ils))
  | NCmd (_, "sup", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Sup ils))
  | NCmd (_, "fun", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Fun ils))
  | NCmd (_, "math", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Math_span ils))
  | NCmd (_, "normal", _, Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Normal ils))
  ...
  | NCmd (_, "href", _, Arg_url (_, url) :: Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Link (url, ils)))
  | NCmd (_, "nameref", _, Arg_symbol (_, r) :: _) ->
    Ok (CInline (Nameref r))
  ...
  | NCmd (_, "marginnote", _, Arg_symbol (_, anchor) :: Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Margin_note (anchor, ils)))
  | NCmd (_, "sidenote", _, Arg_symbol (_, anchor) :: Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CInline (Side_note (anchor, ils)))
  | NCmd (_, "advice", _, Arg_symbol (_, anchor) :: Arg_nodes (_, ns) :: _) ->
    let* ils = elaborate_inlines ns in Ok (CBlock (Advice (anchor, ils)))
  ...
  | NCmd (_, name, _, _) when SSet.mem name mathml_subcmd_set ->
    Ok CSkip
  ...
  | NCmd (_, name, _, _) when SSet.mem name metadata_cmd_set ->
    Ok CSkip

  | NEnv (_, _, "abstract", _, body) ->
    let* bs = build_blocks body in Ok (CBlock (Abstract bs))
  ...
  | NEnv (_, _, "figure", opts, body) ->
    let* bs = build_blocks body in Ok (CBlock (Figure (opts, bs)))
  ...
  | NEnv (_, _, "verbatim", opts, body) ->
    let txt = String.concat ""
      (List.map (function NText (_, t) -> t | _ -> "") body) in
  ...
  | NCmd (pos, name, _, _) ->
    elab_error pos ("unknown command: \\" ^ name)
  | NEnv (pos, _, name, _, _) ->
    elab_error pos ("unknown environment: " ^ name)
  ...
  let rec go acc = function
    ...
    | n :: rest ->
      ...
      (match cls with
       ...
       | CBlock (Image (opts, path)) -> go (Image_inline (opts, path) :: acc) rest
       | _ -> go acc rest)
  in
  go [] ns
...
and elaborate_code_inlines (ns : node list) : inline list result_ =
  let rec go acc = function
    ...
    | NGroup (_, body) :: rest ->
      let* ils = elaborate_code_inlines body in
      let il = match ils with [il] -> il | _ -> Str (render_inlines_to_text ils) in
      go (il :: acc) rest
    | NMath (_, disp, mnodes) :: rest ->
      go (Math (disp, mnodes) :: acc) rest
    | NQuotation (_, body) :: rest ->
      let* ils = elaborate_code_inlines body in
      go (Quotation ils :: acc) rest
    | NCmd (_, "b", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Strong ils :: acc) rest
    | NCmd (_, "emph", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Emph ils :: acc) rest
    | NCmd (_, "u", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Underline ils :: acc) rest
    | NCmd (_, "textsc", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Small_caps ils :: acc) rest
    | NCmd (_, "strikethrough", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Strikethrough ils :: acc) rest
    | NCmd (_, "code", opts, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Code (opts, ils) :: acc) rest
    | NCmd (_, "kbd", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Kbd ils :: acc) rest
    | NCmd (_, "sub", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Sub ils :: acc) rest
    | NCmd (_, "sup", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Sup ils :: acc) rest
    | NCmd (_, "href", _, Arg_url (_, url) :: Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Link (url, ils) :: acc) rest
    | NCmd (_, "fun", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Fun ils :: acc) rest
    | NCmd (_, "math", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Math_span ils :: acc) rest
    | NCmd (_, "normal", _, Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Normal ils :: acc) rest
    | NCmd (_, "label", _, Arg_symbol (_, anchor) :: _) :: rest ->
      go (Anchor anchor :: acc) rest
    | NCmd (_, "newline", _, _) :: rest ->
      go (Line_break :: acc) rest
    | NCmd (_, "numspace", _, _) :: rest ->
      go (Numeric_space :: acc) rest
    | NCmd (_, "qed", _, _) :: rest ->
      go (Str "\xE2\x88\x8E" :: acc) rest
    | NCmd (_, "hrule", _, _) :: rest ->
      go (Horizontal_rule :: acc) rest
    | NCmd (_, "marginnote", _, Arg_symbol (_, anchor) :: Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Margin_note (anchor, ils) :: acc) rest
    | NCmd (_, "sidenote", _, Arg_symbol (_, anchor) :: Arg_nodes (_, ns) :: _) :: rest ->
      let* ils = elaborate_code_inlines ns in go (Side_note (anchor, ils) :: acc) rest
  ...
  let flush_para_rev inlines_acc blocks_rev =
    if inlines_acc = [] then blocks_rev
      ...
      let has_para_break_before_first_lb =
        ...
        find ordered
      in
      if not has_para_break_before_first_lb
         && is_block_level_only_content trimmed
      then
        FB (Plain trimmed) :: blocks_rev
      else
...
and split_list_items (ns : node list) : block list list result_ =
  ...
  let rec split acc cur = function
    ...
    | n :: rest when is_item n ->
      split (List.rev cur :: acc) [] rest
    | n :: rest ->
      split acc (n :: cur) rest
  in
...
and split_description_items (ns : node list) : (inline list * block list) list result_ =
  let rec go = function
    | [] -> Ok []
    | NCmd (_, "term", _, Arg_nodes (_, dt) :: Arg_nodes (_, dd) :: _) :: rest ->
      let* dt_ils = elaborate_inlines dt in
      let* dd_blocks = build_blocks dd in
      let dd_wrapped = match dd_blocks with
        | [Para ils] -> [Plain ils]
        | _ -> dd_blocks
      in
      let* more = go rest in
...
and elaborate_table _name opts spec rows : table_def result_ =
  ...
  let elaborate_row (r : row) : table_row result_ =
    ...
    let top =
      r.row_borders = Border_top || r.row_borders = Border_both
    in
    let bot =
      r.row_borders = Border_bottom || r.row_borders = Border_both
    in
  ...
  let rec map_m f = function
    ...
    | x :: xs ->
      ...
      Ok (y :: ys)
  in
  let* header = match header_row with
    | None -> Ok None
    | Some r -> let* r' = elaborate_row r in Ok (Some r')
  in
...
let is_subsection_mark = function
  | SubsectionMark _ -> true
  | _ -> false
