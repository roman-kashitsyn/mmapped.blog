...
let attr (key : string) (value : string) : attribute = fun buf ->
  Buffer.add_char buf ' ';
...
let parent (tag : string) (attrs : attribute list) (body : t) : t = fun buf ->
  Buffer.add_char buf '<';
...
let leaf (tag : string) (attrs : attribute list) : t = fun buf ->
  Buffer.add_char buf '<';
  Buffer.add_string buf tag;
  render_attrs attrs buf;
  Buffer.add_char buf '>'
